...
- **Customer deployment verification** — `pack-release.sh --verify <URL>` polls `<URL>/version` after pushing until the customer's running build matches the pushed commit. Repeatable (pass `--verify` multiple times for several customers), with a configurable `--verify-timeout` (default 5 minutes). Prints per-customer OK / FAIL summary at the end. Useful for confirming Coolify redeploys landed.
