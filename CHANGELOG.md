...
- **`class="managed-table"` is now the single marker** turning on all three pillars. `sortable-table` / `exportable-table` remain accepted aliases so no existing template broke.
- **New [webapp/static/js/table-core.js](webapp/static/js/table-core.js)** — loaded before the three pillars and owns what all of them needed: the marker/opt-out contract, the shared table-anatomy helpers (`headerRow` / `dataColumns` / `partition`), and the row-visibility arbiter. table-sort.js, table-search.js and table-export.js were refactored onto it, deleting three copies each of the header-row lookup and the data-row/detail-row grouping. One definition of "what is a data row" now serves sorting, searching and exporting — a column excluded in one is excluded in all.
- **Per-pillar opt-out** via a **table-level** `data-no-sort` / `data-no-search` / `data-no-export`, so a table with a bespoke implementation of one pillar keeps it and gains the other two instead of being excluded wholesale.
- **`FERowFilter` row-visibility arbiter** — the enabling piece. Pages that filter rows with their own controls (leases state chips, scan reachability buttons, TLS activity status buttons, scan-compare "Diff only") previously wrote `row.style.display` directly; adding a second filter source would mean the last writer silently undid the first — search a filtered table and the filter evaporates. Visibility is now a **consensus**: each source registers a veto under its own key and a row shows only when nobody vetoes it. Converted callers: leases (`leaseState`), scan tool (`resultState`), scan compare (`diffOnly`), TLS activity (`logStatus`). **Standing rule added: page-specific row filtering goes through `FERowFilter.set()`, never `style.display`.**
- **Coverage audit of all 90 `<table>` elements across 44 templates.** Every data-listing table is now managed (~61 total); fixed-size key/value, settings-form, detail and legend tables are deliberately excluded — sort/search/export on a 2-column settings panel is noise. Newly managed: DHCP dashboard (3), DHCP credentials list, TLS deployments + activity log, migration parsed (13), migration dedup (2), migration rules/NAT/VPN (3), sandbox (2), scan-history compare, engines cluster detail (4), scan tool results, rules viewer, scan-history detail.
- **Bespoke pillars preserved** rather than ripped out where they do more than the generic one: the cluster-detail cross-tab search with `<mark>` highlighting, the rules viewer's section-aware search, the scan-history detail sorter + state filters, and the two IP-aware bespoke sorters. Section-structured tables (rules viewer, migration rules/NAT/VPN) opt out of *sorting* specifically, because sorting would scramble the sections that give the rows their meaning.
- **Fixed a form-submission hazard** the search introduced: managed tables nested inside a `<form>` (the leases viewer sits in the "Add to reservations" form) would have submitted it on Enter in a search box. Injected inputs now carry no `name` and swallow Enter, so they can neither submit the form nor be posted as fields.
- **Fixed a pre-existing bug** in the TLS activity log: filtering by status hid the log row but left its collapsible detail row on screen. Detail rows now follow their parent.
- **Verified with jsdom** (18 assertions, all passing): the marker contract and each table-level opt-out in isolation; legacy `sortable-table` still gets all three; **filter and search intersect rather than replace**, clearing either restores the other; export sees exactly the visible intersection and omits checkbox/Actions columns; detail rows still travel with their parent through a sort; and the produced .xlsx still loads in openpyxl with no warnings.
...
- **New shared asset:** [webapp/static/js/table-export.js](webapp/static/js/table-export.js) (styles appended to [webapp/static/css/table-sort.css](webapp/static/css/table-sort.css)), loaded globally from base.html **after** table-search.js so it can dock its buttons into that module's search bar. Auto-inits every marked table (all ~22 listing tables got export for free). `window.FEATableExport.enhance(el)` for AJAX tables. *(Superseded same day by the unified `managed-table` marker + table-core.js — see the entry above.)*
...
- **New shared asset:** [webapp/static/js/table-search.js](webapp/static/js/table-search.js) (styles in [webapp/static/css/table-sort.css](webapp/static/css/table-sort.css)), loaded globally from base.html; auto-inits every `table.sortable-table`, `window.FEATableSearch.enhance(el)` for AJAX tables.
- **Two searches per table** (per operator decision — global box + per-column row): a **global box** above the table matching across ALL data columns at once, plus a **per-column filter row** (`<tr class="fe-filter-row">` under the header) with one input per data column for selective per-field filtering. Active per-column filters combine with AND.
- **Token grammar** (both the global box and each column box): whitespace separates tokens; **default logical OR** (match any token), a **capital `AND`** token switches that box to **logical AND** (match every token); lowercase "and" is a literal term. Case-insensitive substring. Live "M of N" counter by the global box.
- **Column opt-out:** `data-no-sort` columns (checkbox / Actions) get no filter input and are excluded from the global match, so search never keys off button labels.
- **Interop:** table-sort.js skips the injected `fe-filter-row`; detail/continuation rows (e.g. a `push_failed` reservation's `colspan` error row) hide/show with their parent. Verified end-to-end with jsdom (global OR/AND, per-column AND, detail-row visibility, sort coexistence).
- **Existing search audited:** none of the retrofitted listing tables had a client-side row-search widget to replace. `/logs` has a *server-side* filter form (feature/level/status/time) that narrows the DB query before pagination — a different mechanism, left intact; the client-side search layers on the current page. Bespoke-search tables (leases, scan tool, scan-history detail, cluster-detail, rules viewer) keep their specialised search.
- Caveat (same as sort): client-side over rendered rows → on server-paginated tables it searches the current page only. Standing rule added to CLAUDE.md § Table search.
...
- **Zero-config, auto-typed:** every non-empty header becomes clickable (▲/▼ indicator, click toggles asc/desc). Column type is auto-detected per column — **ip** (octet-aware: `192.168.1.6` before `192.168.1.69`, CIDR respected), **num**, **date**, else **text** (locale/numeric). Empty / `—` cells always sort to the bottom. Only the first `<tbody>` is sorted.
- **Detail-row aware:** interleaved non-data rows (e.g. the `<td colspan>` SMC-error row rendered under a `push_failed` reservation on the DHCP scope-detail table) travel *with* their parent data row when sorting — they're never orphaned or floated to the top. Rows are grouped by column count: a data row plus any following non-data rows sort as a unit.
