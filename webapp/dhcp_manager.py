...
from webapp.smc_dhcp_client import (
    ...
    list_cluster_nodes, dump_engine_interfaces,
    host_create, host_update, host_get, host_list_by_scope,
    is_valid_mac, normalize_mac,
    _derive_cidr_from_pool,
)
from webapp.smc_tls_client import smc_error_detail
# Credential-feature SSH primitives (SSHTarget/SSHCredential) are used by
# the _cred_to_target / _cred_to_payload helpers below, which the leases
# viewer + subnet scan still call. The rule-lifecycle + verify primitives
# moved to webapp/credentials_manager.py with the wizard routes.
from webapp.dhcp_ssh import SSHTarget, SSHCredential, get_file as ssh_get_file
...
# ── SSH credentials — feature EXTRACTED to webapp/credentials_manager.py ─
#
# (2026-06-12) The credential wizard, source-IP management, SSH-allow-rule
# lifecycle and per-node bootstrap/apply/verify/delete routes live in
# webapp/credentials_manager.py now. They still register on THIS dhcp_bp
# (imported at the bottom of this file), so URLs (/dhcp/credentials/*) and
# endpoint names (dhcp.credentials_*) are unchanged. The three helpers
# above (_cred_to_target / _cred_to_payload / _operator_email) stay here
# because the leases viewer + subnet scan below use them too.
...
# ── Credential Manager registration ─────────────────────────────────────
