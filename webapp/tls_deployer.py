...
def execute_deployment_inside_session(dep: TLSDeployment,
                                      cert_fullchain: str,
                                      cert_privkey: str) -> DeployResult:
    """Run the 5-step TLS pipeline assuming an SMC session is already open.

    Used by:
      * the queue handler (`shared/queue_runner.py:_handle_deploy_tls`),
        which runs inside a session opened by the queue runner;
      * `execute_deployment()` below, which opens its own session and
        delegates here for the orchestration body.

    Steps:
      1. Import TLS credential (delete-and-recreate)
      2. Create source/dest Host elements
      3. Assign credential to engine TLS inspection
      4. Find-or-create policy rule
      5. Upload policy to engine

    Returns a populated `DeployResult` with `steps[]` for the queue UI's
...
def execute_deployment(dep: TLSDeployment, smc_cfg: SMCConfig,
    ...
    """Open an SMC session and delegate to ``execute_deployment_inside_session``.

    Kept for callers that don't already own a session (legacy callers,
    standalone scripts). Phase E.2 routes go through the queue runner
    which opens the session itself.
    """
    result = DeployResult()
    try:
        with smc_session(smc_cfg):
            return execute_deployment_inside_session(
                dep, cert_fullchain, cert_privkey,
            )
    except Exception as e:
